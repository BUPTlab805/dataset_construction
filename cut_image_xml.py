...
def calculate_images(img_h,img_w,delta_w,delta_h):
    # 计算图像剪切H
    h_index = int(img_h / delta_h)
    tmp_h = img_h - int(h_index * delta_h)
    if tmp_h < 500:
        h_index = h_index - 1
        tmp_h = img_h - int(h_index * delta_h)
    # 计算图像剪切W
    w_index = int(img_w / delta_w)
    tmp_w = img_w - int(w_index * delta_w)
    if tmp_w < 500:
        w_index = w_index - 1
        tmp_w = img_w - int(w_index * delta_w)
    ...
    for x_i in range(int(w_index+1)):
            ...
            for y_i in range(int(h_index+1)):
                ...
                for j in range(len(x1)):
                    ...
                    # 边界框被横着的切割线穿过
                    elif (x1[j] >= tmp_ox) & (y1[j] >= tmp_oy) & (x2[j] <= x_boundary) & (y2[j] > y_boundary) & (y1[j] < y_boundary):
                    ...
                    # 边界框被竖着的切割线切割
                    elif (x1[j] >= tmp_ox) & (y1[j] >= tmp_oy) & (x2[j] > x_boundary) & (y2[j] <= y_boundary) & (x1[j] < x_boundary):
                    ...
                    # 最不理想的情况，边界框被两条切割线穿过
                    elif (x1[j] >= tmp_ox) & (y1[j] >= tmp_oy) & (x2[j] >= x_boundary) & (y2[j] >= y_boundary)& (x1[j]< x_boundary)&(y1[j]<y_boundary):
...
if __name__ == '__main__':
    num_begin = 6136
    path = "/home/ubuntu/lijiangtao/ImageDataset/insect_img_dataset/pascal_imgset/VOCmobile/VOC2007/"
    ...
    for i, txt in enumerate(txts):
        ...
        for line in lines:
            ...
            num_end, new_imgnames = read_xml(img_path=img_path, xmlpath=xml_path, outputpath=outputpath, delta_w=1024,
                                             delta_h=1024, num_begin=num_begin)
            num_begin = num_end + num_begin
            print(num_begin, num_end)
            for new_imgname in new_imgnames:
                f.writelines(new_imgname[:-4] + '\r')
        print('_____________________',txt)
    # num_begin = 0
    # xmls_path = r'C:\Users\ljt\Desktop\VOC2007\Annotations'
    # img_path = r'C:\Users\ljt\Desktop\VOC2007\JPEGImages'
    # outputpath = r'C:\Users\ljt\Desktop\VOC2007\new'
    ...
    #     print(xml_path)
    #     num_end = read_xml(img_path,xml_path,outputpath,512,512,num_begin)
    #     num_begin = num_end + num_begin
    #     print(num_begin,num_end)
